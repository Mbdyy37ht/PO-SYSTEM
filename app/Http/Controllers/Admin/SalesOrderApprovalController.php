...
class SalesOrderApprovalController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($query)
                ->addColumn('action', function ($po) {
                    $reviewBtn = '<a href="' . route('admin.sales-orders.show', $po) . '" class="inline-flex items-center px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-medium rounded-lg transition-colors duration-200">
                ...
                ->addColumn('po_info', function ($po) {
                    return '<div class="flex items-center">
                        <div>
                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">' . $po->so_number . '</div>
                ...
                ->addColumn('customer_info', function ($po) {
                    return '<div class="text-sm text-gray-900 dark:text-gray-100">' . $po->customer->name . '</div>
                    <div class="text-xs text-gray-500 dark:text-gray-400">' . $po->customer->code . '</div>';
                })
                ...
                ->rawColumns(['action', 'po_info', 'customer_info', 'creator_info', 'total_amount', 'priority', 'status'])
                ->make(true);
        }

        // For statistics (non-AJAX request)
        $pendingCount = SalesOrder::where('status', 'pending')->count();
        $totalPendingAmount = SalesOrder::where('status', 'pending')->sum('total_amount');
        $avgPendingAmount = SalesOrder::where('status', 'pending')->avg('total_amount');
        $highPriorityCount = SalesOrder::where('status', 'pending')->where('total_amount', '>=', 25000000)->count();

        return view('admin.sales-orders.approval', compact('pendingCount', 'totalPendingAmount', 'avgPendingAmount', 'highPriorityCount'));
    }
    ...
    public function approve(Request $request, SalesOrder $salesOrder)
    {
        ...
        // VALIDASI STOCK sebelum approve
        $salesOrder->load(['details.item', 'warehouse']);
        $stockErrors = [];
        ...
        foreach ($salesOrder->details as $detail) {
            ...
            if ($detail->quantity > $availableStock) {
                $stockErrors[] = "{$detail->item->name}: Requested {$detail->quantity}, Available {$availableStock}";
            }
        }
