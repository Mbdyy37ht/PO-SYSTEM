<x-app-layout>
    ...
    <div class="space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200">Review Purchase Order</h2>
                <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-400">Review and approve/reject this purchase order</p>
            </div>
        ...
        @if($purchaseOrder->total_amount >= 50000000)
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-red-700 dark:text-red-300">
                            <strong>⚠️ CRITICAL VALUE ALERT!</strong> This purchase order has a very high value (≥ Rp 50,000,000). Please review carefully before approval.
                        </p>
                    </div>
                </div>
            </div>
        @elseif($purchaseOrder->total_amount >= 25000000)
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-orange-700 dark:text-orange-300">
                            <strong>⚡ HIGH VALUE PO:</strong> This purchase order has a high value (≥ Rp 25,000,000). Please verify all details.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-blue-700 dark:text-blue-300">
                            This purchase order has already been <strong>{{ $purchaseOrder->status }}</strong>. No further action is required.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg overflow-hidden">
            <div class="px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                <h3 class="text-base sm:text-lg font-semibold text-gray-800 dark:text-gray-200">Purchase Order Information</h3>
            </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-lg font-medium text-gray-900 dark:text-gray-100">{{ $purchaseOrder->supplier->name }}</p>
                            <div class="mt-1 space-y-1">
                                <p class="text-sm text-gray-600 dark:text-gray-400">
                                    <span class="font-medium">Code:</span> {{ $purchaseOrder->supplier->code }}
                                </p>
                                @if($purchaseOrder->supplier->phone)
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        <span class="font-medium">Phone:</span> {{ $purchaseOrder->supplier->phone }}
                                    </p>
                                @endif
                                ...
                                @if($purchaseOrder->supplier->contact_person)
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        <span class="font-medium">Contact:</span> {{ $purchaseOrder->supplier->contact_person }}
                                    </p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $purchaseOrder->warehouse->code }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                            ...
                            <div class="mt-1 flex items-center">
                                <div class="flex-shrink-0 h-10 w-10 bg-blue-100 dark:bg-blue-900 rounded-full flex items-center justify-center">
                                    <span class="text-sm font-medium text-blue-800 dark:text-blue-300">
                                        {{ strtoupper(substr($purchaseOrder->creator->name, 0, 2)) }}
                                    </span>
                                </div>
                                <div class="ml-3">
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $purchaseOrder->creator->name }}</p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">{{ $purchaseOrder->creator->email }}</p>
                                </div>
                            </div>
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $purchaseOrder->created_at->format('d M Y, H:i') }}</p>
                        </div>
                        ...
                        @if($purchaseOrder->approver)
                            <div>
                                ...
                                <div class="mt-1 flex items-center">
                                    <div class="flex-shrink-0 h-10 w-10 {{ $purchaseOrder->status === 'approved' ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900' }} rounded-full flex items-center justify-center">
                                        <span class="text-sm font-medium {{ $purchaseOrder->status === 'approved' ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300' }}">
                                            {{ strtoupper(substr($purchaseOrder->approver->name, 0, 2)) }}
                                        </span>
                                    </div>
                                    <div class="ml-3">
                                        <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $purchaseOrder->approver->name }}</p>
                                        <p class="text-xs text-gray-600 dark:text-gray-400">{{ $purchaseOrder->approver->email }}</p>
                                    </div>
                                </div>
                        ...
                        @if($purchaseOrder->approval_notes)
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">
                                    {{ $purchaseOrder->status === 'approved' ? 'Approval Notes' : 'Rejection Reason' }}
                                </label>
                                <p class="mt-1 text-sm {{ $purchaseOrder->status === 'rejected' ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-gray-100' }}">
                        ...
                        @if($purchaseOrder->notes)
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $purchaseOrder->notes }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                                    ...
                                    <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                        <div>
                                            <p class="text-sm text-gray-900 dark:text-gray-100">
                                                Purchase Order <span class="font-medium">created</span> by 
                                                <span class="font-medium">{{ $purchaseOrder->creator->name }}</span>
                                            </p>
                                        </div>
                                        <div class="whitespace-nowrap text-right text-sm text-gray-500 dark:text-gray-400">
                                            <time datetime="{{ $purchaseOrder->created_at }}">{{ $purchaseOrder->created_at->format('d M Y, H:i') }}</time>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="relative pb-8">
                                    ...
                                    <div class="relative flex space-x-3">
                                        ...
                                        <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                            ...
                                            <div class="whitespace-nowrap text-right text-sm text-gray-500 dark:text-gray-400">
                                                <time>{{ $purchaseOrder->created_at->format('d M Y, H:i') }}</time>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @if($purchaseOrder->approved_at)
                            <li>
                                <div class="relative pb-8">
                                    <div class="relative flex space-x-3">
                                        <div>
                                            <span class="h-8 w-8 rounded-full {{ $purchaseOrder->status === 'approved' ? 'bg-green-500' : 'bg-red-500' }} flex items-center justify-center ring-8 ring-white dark:ring-gray-800">
                                        ...
                                        <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                                            <div>
                                                <p class="text-sm text-gray-900 dark:text-gray-100">
                                                    Purchase Order <span class="font-medium {{ $purchaseOrder->status === 'approved' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">{{ $purchaseOrder->status }}</span> by 
                                                    <span class="font-medium">{{ $purchaseOrder->approver->name }}</span>
                                                </p>
                                                @if($purchaseOrder->approval_notes)
                                            ...
                                            <div class="whitespace-nowrap text-right text-sm text-gray-500 dark:text-gray-400">
                                                <time datetime="{{ $purchaseOrder->approved_at }}">{{ \Carbon\Carbon::parse($purchaseOrder->approved_at)->format('d M Y, H:i') }}</time>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white dark:bg-gray-800">
            <div class="mt-3">
                ...
                <div class="mt-3 text-center">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Approve Purchase Order</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                            Are you sure you want to approve this purchase order?
                        </p>
                        <p class="text-sm font-semibold text-gray-900 dark:text-gray-100 mt-2">
                            {{ $purchaseOrder->po_number }} - Rp {{ number_format($purchaseOrder->total_amount, 0, ',', '.') }}
                        </p>
                        
                        <form action="{{ route('admin.purchase-orders.approve', $purchaseOrder) }}" method="POST" class="mt-4">
                            @csrf
                            <div class="text-left">
    ...
    <div id="rejectModal" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white dark:bg-gray-800">
            <div class="mt-3">
                ...
                <div class="mt-3 text-center">
                    <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-gray-100">Reject Purchase Order</h3>
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-600 dark:text-gray-400">
                        ...
                        <form action="{{ route('admin.purchase-orders.reject', $purchaseOrder) }}" method="POST" class="mt-4">
                            @csrf
                            <div class="text-left">
