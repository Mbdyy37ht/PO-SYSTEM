<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Sales Order Details</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">View sales order information</p>
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                @if($salesOrder->status === 'draft')
                    <a href="{{ route('staff.sales-orders.edit', $salesOrder) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $salesOrder->customer->name }}</p>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $salesOrder->creator->name }}</p>
                            <p class="text-sm text-gray-600 dark:text-gray-400">{{ $salesOrder->created_at->format('d M Y, H:i') }}</p>
                        </div>
                        ...
                        @if($salesOrder->approver)
                            <div>
                                ...
                                <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $salesOrder->approver->name }}</p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $salesOrder->approval_date ? \Carbon\Carbon::parse($salesOrder->approval_date)->format('d M Y, H:i') : '-' }}</p>
                            </div>
                        @endif

                        @if($salesOrder->rejection_reason)
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Rejection Reason</label>
                                <p class="mt-1 text-red-600 dark:text-red-400">{{ $salesOrder->rejection_reason }}</p>
                            </div>
                        @endif
                        ...
                        @if($salesOrder->notes)
                            <div>
                                ...
                                <p class="mt-1 text-gray-900 dark:text-gray-100">{{ $salesOrder->notes }}</p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-start">
                            ...
                            <div class="ml-4">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400">{{ $salesOrder->created_at->format('d M Y, H:i') }}</p>
                                <p class="text-sm text-gray-600 dark:text-gray-400">by {{ $salesOrder->creator->name }}</p>
                            </div>
                        </div>

                        @if($salesOrder->approval_date)
                            <div class="flex items-start">
                                <div class="flex-shrink-0">
                                    <div class="flex items-center justify-center h-10 w-10 rounded-full {{ $salesOrder->status === 'approved' ? 'bg-green-100 dark:bg-green-900' : 'bg-red-100 dark:bg-red-900' }}">
                                        <svg class="h-6 w-6 {{ $salesOrder->status === 'approved' ? 'text-green-600 dark:text-green-300' : 'text-red-600 dark:text-red-300' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <div class="ml-4">
                                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ ucfirst($salesOrder->status) }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ \Carbon\Carbon::parse($salesOrder->approval_date)->format('d M Y, H:i') }}</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">by {{ $salesOrder->approver->name }}</p>
                                </div>
                            </div>
                        @endif
                    </div>
