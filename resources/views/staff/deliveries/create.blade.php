<x-app-layout>
    ...
    <div class="space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
                <h2 class="text-xl sm:text-2xl font-bold text-gray-800 dark:text-gray-200">Create Delivery</h2>
                <p class="mt-1 text-xs sm:text-sm text-gray-600 dark:text-gray-400">Record goods delivered from customer</p>
            </div>
        ...
        @if(session('success'))
            <div class="bg-green-50 dark:bg-green-900 border-l-4 border-green-400 p-3 sm:p-4 rounded">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-xs sm:text-sm text-green-700 dark:text-green-300">{{ session('success') }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if(session('error'))
            <div class="bg-red-50 dark:bg-red-900 border-l-4 border-red-400 p-3 sm:p-4 rounded">
                <div class="flex">
                    <div class="flex-shrink-0">
                    ...
                    <div class="ml-3">
                        <p class="text-xs sm:text-sm text-red-700 dark:text-red-300">{{ session('error') }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if($errors->any())
            <div class="bg-red-50 dark:bg-red-900 border-l-4 border-red-400 p-3 sm:p-4 rounded">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-xs sm:text-sm font-medium text-red-700 dark:text-red-300">There were some errors with your submission:</p>
                        <ul class="mt-2 text-xs sm:text-sm text-red-700 dark:text-red-300 list-disc list-inside">
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-xs sm:text-sm text-yellow-700 dark:text-yellow-300">
                            <strong>No Approved Sales Orders Available</strong><br>
                            There are no approved Sales Orders without Deliverys. Please create and approve a Sales Order first before creating a GRN.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <!-- Create Form -->
        <form action="{{ route('staff.deliveries.store') }}" method="POST" id="grnForm">
            @csrf

            <div class="space-y-4 sm:space-y-6">
                <!-- Delivery Information Card -->
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-4 sm:p-6">
                    <h3 class="text-base sm:text-lg font-semibold text-gray-800 dark:text-gray-200 mb-4">Delivery Information</h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            <select name="sales_order_id" id="sales_order_id" required onchange="loadPODetails()"
                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 text-sm sm:text-base"
                                {{ $salesOrders->isEmpty() ? 'disabled' : '' }}>
                                <option value="">Select Sales Order</option>
                                @foreach($salesOrders as $po)
                                    <option value="{{ $po->id }}" data-customer="{{ $po->customer->name }}"
                                        data-warehouse="{{ $po->warehouse_id }}"
                                        {{ old('sales_order_id') == $po->id ? 'selected' : '' }}>
                                        {{ $po->so_number }} - {{ $po->customer->name }} ({{ \Carbon\Carbon::parse($po->po_date)->format('d M Y') }})
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <tbody id="itemsTableBody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                                        Select a Sales Order and click "Load PO Items" to add items
                                    </td>
                                </tr>
                            </tbody>
                        </table>
    ...
    <script>
        ...
        function clearItems() {
            ...
            tbody.innerHTML = '<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Select a Sales Order and click "Load PO Items" to add items</td></tr>';

            const cardView = document.getElementById('itemsCardView');
            cardView.innerHTML = '<div class="text-center text-sm text-gray-500 dark:text-gray-400 py-8">Select a Sales Order and click "Load PO Items" to add items</div>';
        }

        function loadPOItems() {
            const poId = document.getElementById('sales_order_id').value;
            if (!poId) {
        ...
        function renderItems(items) {
            ...
            if (items.length === 0) {
                tbody.innerHTML = '<tr><td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No items found in the selected Sales Order</td></tr>';
                cardView.innerHTML = '<div class="text-center text-sm text-gray-500 dark:text-gray-400 py-8">No items found in the selected Sales Order</div>';
                return;
            }
            ...
            items.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-4 text-center">
                        <input type="number" name="items[${index}][quantity_delivered]" value="${item.quantity}" min="0" required
                ...
                card.innerHTML = `
                    ...
                    <div class="grid grid-cols-2 gap-3 mb-3">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Qty Delivered <span class="text-red-500">*</span></label>
                            <input type="number" name="items_mobile[${index}][quantity_delivered]" value="${item.quantity}" min="0" required
