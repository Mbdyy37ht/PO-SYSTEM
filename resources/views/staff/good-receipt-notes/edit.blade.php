<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Good Receipt Note</h2>
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">{{ $goodReceiptNote->grn_number }}</p>
            </div>
            <a href="{{ route('staff.good-receipt-notes.show', $goodReceiptNote) }}"
        ...
        <!-- Status Alert -->
        @if($goodReceiptNote->status === 'rejected')
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-red-700 dark:text-red-300">
                            <strong>This GRN was rejected.</strong><br>
                            Reason: {{ $goodReceiptNote->approval_notes ?? 'No reason provided' }}<br>
        ...
        <!-- Edit Form -->
        <form action="{{ route('staff.good-receipt-notes.update', $goodReceiptNote) }}" method="POST" id="grnEditForm">
            @csrf
            @method('PUT')
                ...
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $goodReceiptNote->grn_number }}" readonly
                        ...
                        <div>
                            ...
                            <input type="date" name="grn_date" id="grn_date"
                                value="{{ old('grn_date', $goodReceiptNote->grn_date->format('Y-m-d')) }}" required
                        ...
                        <div>
                            ...
                            <input type="hidden" name="purchase_order_id" value="{{ $goodReceiptNote->purchase_order_id }}">
                            <input type="text" value="{{ $goodReceiptNote->purchaseOrder->po_number }} - {{ $goodReceiptNote->purchaseOrder->supplier->name }}" readonly
                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 bg-gray-100 shadow-sm">
                        </div>
                            ...
                            <select name="warehouse_id" id="warehouse_id" required
                                ...
                                @foreach($warehouses as $warehouse)
                                    <option value="{{ $warehouse->id }}"
                                        {{ old('warehouse_id', $goodReceiptNote->warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500">{{ old('notes', $goodReceiptNote->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tbody id="itemsTableBody" class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach($goodReceiptNote->details as $index => $detail)
        ...
        <div class="mt-6">
            <form action="{{ route('staff.good-receipt-notes.destroy', $goodReceiptNote) }}" method="POST"
                onsubmit="return confirm('Are you sure you want to delete this Good Receipt Note? This action cannot be undone.');">
                @csrf
                @method('DELETE')
        ...
        function renderItems(items) {
            ...
            items.forEach((item, index) => {
                ...
                row.innerHTML = `
                    ...
                    <td class="px-4 py-4 text-center">
                        <input type="number" name="items[${index}][quantity_received]" value="${item.quantity_received}" min="0" required
                            class="w-24 text-center rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 focus:border-green-500 focus:ring-green-500">
                    </td>
                    <td class="px-4 py-4">
