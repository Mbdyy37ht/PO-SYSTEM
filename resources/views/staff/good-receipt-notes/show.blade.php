    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Good Receipt Note Details</p>
            </div>
            <div class="flex items-center space-x-2">
                ...
                @if($goodReceiptNote->status === 'draft')
                    <a href="{{ route('staff.good-receipt-notes.edit', $goodReceiptNote) }}"
        ...
        <div class="flex items-center space-x-4">
            ...
            @elseif($goodReceiptNote->status === 'pending')
                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <p class="text-sm text-red-700 dark:text-red-300">
                            <strong>This GRN has been rejected and is now read-only.</strong><br>
                            Reason: {{ $goodReceiptNote->approval_notes ?? 'No reason provided' }}<br>
                            <span class="text-xs mt-2 block">To fix this issue, please create a new GRN from the same Purchase Order with the correct information.</span>
                        </p>
                    </div>
                </div>
            </div>
        ...
        @if($goodReceiptNote->status === 'draft')
            <div class="flex justify-end space-x-4">
                <form action="{{ route('staff.good-receipt-notes.destroy', $goodReceiptNote) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this Good Receipt Note? This action cannot be undone.');">
                    @csrf
                    @method('DELETE')
