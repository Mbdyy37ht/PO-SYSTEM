<aside
    ...
    <!-- Sidebar Header -->
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 border-b border-gray-200 dark:border-gray-700">
        <div class="flex items-center min-w-0">
            <x-application-logo class="block h-8 w-auto flex-shrink-0 fill-current text-gray-800 dark:text-gray-200" />
            <span class="ml-2 text-lg sm:text-xl font-semibold text-gray-800 dark:text-gray-200 truncate">WMS</span>
        </div>
        <!-- Close button for mobile -->
        <button @click="sidebarOpen = false" class="lg:hidden flex-shrink-0 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none ml-2">
    ...
    <!-- Sidebar Navigation -->
    <nav class="flex-1 px-2 sm:px-4 py-4 sm:py-6 overflow-y-auto h-[calc(100vh-8rem)]">
        @if(Auth::user()->role && in_array(Auth::user()->role->name, ['admin', 'manager']))
            <!-- Admin/Manager Navigation -->
            <div class="space-y-1 sm:space-y-2">
                <!-- Dashboard -->
                <a href="{{ route('admin.dashboard') }}"
                   class="flex items-center px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-medium rounded-lg transition-colors duration-200
                ...
                <div x-data="{
                    open: {{ request()->routeIs('admin.items.*') || request()->routeIs('admin.suppliers.*') || request()->routeIs('admin.warehouses.*') || request()->routeIs('admin.customers.*') ? 'true' : 'false' }}
                }" class="pt-2 sm:pt-3">
                    <!-- Dropdown Header -->
                    <button @click="open = !open"
                            class="flex items-center justify-between w-full px-3 sm:px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hover:text-gray-700 dark:hover:text-gray-300 transition-colors duration-200">
                ...
                <!-- Approvals Dropdown Section -->
                <div x-data="{
                    open: {{ request()->routeIs('admin.purchase-orders.*') || request()->routeIs('admin.sales-orders.*') || request()->routeIs('admin.good-receipt-notes.*') || request()->routeIs('admin.deliveries.*') ? 'true' : 'false' }}
                }" class="pt-2 sm:pt-3">
                    <!-- Dropdown Header -->
                    <button @click="open = !open"
                            class="flex items-center justify-between w-full px-3 sm:px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider hover:text-gray-700 dark:hover:text-gray-300 transition-colors duration-200">
                    ...
                    <div x-show="open"
                        ...
                        <a href="{{ route('admin.good-receipt-notes.approval') }}"
                           class="flex items-center px-3 sm:px-4 py-2.5 sm:py-3 text-sm font-medium rounded-lg transition-colors duration-200
                            ...
                            <span class="truncate">Good Receipt Notes</span>
                        </a>
        ...
        @elseif(Auth::user()->role && Auth::user()->role->name === 'staff')
            ...
            <div class="space-y-1 sm:space-y-2">
                ...
                <div class="pt-3 sm:pt-4 pb-1 sm:pb-2">
                    <h3 class="px-3 sm:px-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Transactions</h3>
                </div>
    ...
    <div class="absolute bottom-0 left-0 right-0 border-t border-gray-200 dark:border-gray-700 p-3 sm:p-4 bg-white dark:bg-gray-800">
        <div class="flex items-center">
            <div class="flex-shrink-0">
                <div class="w-9 h-9 sm:w-10 sm:h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold text-sm sm:text-base">
                    {{ substr(Auth::user()->name, 0, 1) }}
                </div>
            </div>
            <div class="ml-2 sm:ml-3 flex-1 min-w-0">
                <p class="text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-200 truncate">{{ Auth::user()->name }}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ Auth::user()->role ? ucfirst(Auth::user()->role->name) : 'User' }}</p>
            </div>
            <div x-data="{ open: false }" class="relative flex-shrink-0 ml-2">
